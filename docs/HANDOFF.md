...
Current test count: **1009 tests, 1003 pass, 0 fail, 6 skipped**, green in
...
number hasn't earned. Run `npm test` yourself; when this line was written it was 1009
tests, 1003 pass, 0 fail, 6 skipped (see §1).
...
7. **A restriction needs a second question: what now falls outside it that should not?** Three separate repairs this session were correct in intent, applied at the wrong point, and shipped something worse than the defect they fixed — each one reverted-and-proven, covered by a test, and reviewed. The tests held, because a test confirms the behaviour it was asked to confirm. Narrowing the env-secret detector to stop it corrupting prose also stopped it catching `export DB_PASSWORD=…`, the commonest shape in which a secret is pasted. Filtering unapproved memories out of the session-start block placed the filter after the result limit, so ineligible rows consumed every slot and one user memory among sixty imports produced an empty block. Ask the inverse explicitly — list what the old behaviour caught that the new one does not, and count what still gets through the narrowed gate — because nobody asks it by accident.
